...
const FLOORS = {
  LT1: {
    ...
    navpoints: "geojson/RSUP-NavPoints.geojson"
  },
  LT2: {
...
function buildVisibilityGraph(graph, navPoints, walls, maxDist = 30) {
  ...
  for (let i = 0; i < coords.length; i++) {
    for (let j = i + 1; j < coords.length; j++) {
      ...
      // Optional: limit distance to keep graph clean
      ...
      for (const wall of walls.features) {
        // Check if line crosses any wall boundary
        if (turf.lineIntersect(line, wall).features.length > 0) {
          blocked = true;
          break;
        }
        // Check if the segment is entirely inside a wall polygon
      ...
      if (!blocked) {
        // Ensure we don't skip over any intermediate NavPoints
        for (let k = 0; k < coords.length; k++) {
          if (k === i || k === j) continue;
          const p3 = coords[k];
const distToLine = turf.pointToLineDistance(turf.point(p3), line, { units: 'meters' });
if (distToLine < 0.5) {
  // Only block if P3 is NOT one of the endpoints (with a small buffer)
...
(async function main() {
  ...
  const [walls, walkable, doors, pois, navlines, rooms, navpoints] =
    await Promise.all([
      load(FILES.walls),
      load(FILES.walkable),
      load(FILES.doors),
...
map.addLayer({
  ...
  async function loadFloor(floorKey) {
    ...
    const [walls, walkable, doors, rooms, pois, navlines, navpoints] = await Promise.all([
      ...
      f.navpoints ? fetch(f.navpoints).then(r => r.json()) : Promise.resolve(emptyFC())
    ]);
    ...
    const filteredNavPoints = filterPoisByFloor(navpoints, floorKey);
    ...
    map.getSource("nav-points").setData(filteredNavPoints);
...
if (currentSegment === 1 || currentSegment === 2) {
  ...
  map.getSource("highlight-room")?.setData(emptyFC());
    ...
    if (USE_AUTO_NAV && navpoints.features.length > 0) {
      buildVisibilityGraph(graph, filteredNavPoints, walls);
    } else {
      for (const feat of navlines.features) {
        if (!feat.geometry) continue;
        const g = feat.geometry;
